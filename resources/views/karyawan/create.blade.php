            ...
            <form action="{{ isset($karyawan) ? route('karyawan.update', $karyawan->id) : route('karyawan.store') }}"
                ...
                <div class="row">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                @php
                                    ...
                                    $val_agama = old('agama', $karyawan->detail->agama ?? '');
                                    $is_lainnya = $val_agama != '' && !in_array($val_agama, $list_agama);
                                @endphp
                                <select name="agama" id="agama_select" class="form-select" required>
                                    <option value="">-- Pilih Agama --</option>
                                    @foreach($list_agama as $agm)
                                        <option value="{{ $agm }}" {{ $val_agama == $agm ? 'selected' : '' }}>{{ $agm }}</option>
                                    @endforeach
                                    <option value="Lainnya" {{ $is_lainnya ? 'selected' : '' }}>Lainnya</option>
                                </select>
                                <div id="agama_lainnya_container" class="mt-2 {{ $is_lainnya ? '' : 'd-none' }}">
                                    <input type="text" name="agama_lainnya" id="agama_lainnya_input" class="form-control"
                                        placeholder="Sebutkan agama lainnya..." value="{{ $is_lainnya ? $val_agama : '' }}">
                                </div>
                            </div>
                    ...
                    <script>
                        ...
                        // Handling Agama Lainnya
                        const selectAgama = document.getElementById('agama_select');
                        const containerAgamaLainnya = document.getElementById('agama_lainnya_container');
                        const inputAgamaLainnya = document.getElementById('agama_lainnya_input');

                        function toggleAgamaLainnya() {
                            if (selectAgama.value === 'Lainnya') {
                                containerAgamaLainnya.classList.remove('d-none');
                                inputAgamaLainnya.required = true;
                            } else {
                                containerAgamaLainnya.classList.add('d-none');
                                inputAgamaLainnya.required = false;
                                inputAgamaLainnya.value = '';
                            }
                        }

                        selectAgama.addEventListener('change', function () {
                            toggleAgamaLainnya();
                            if (this.value === 'Lainnya') {
                                inputAgamaLainnya.focus();
                            }
                        });
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-4">
                        <div class="p-3 border border-info rounded bg-info bg-opacity-10 h-100">
                            ...
                            <select name="nama_pt_group" class="form-select border-info mb-2">
                                <option value="">-- Referensi perusahaan --</option>
                                @foreach([
                                        'PT. CHEMCO HARAPAN NUSANTARA',
                                        'PT. ELASTOMIX INDONESIA',
                                        'PT. ARAI RUBBER SEAL INDONESIA',
                                        'PT. FUJIOOZX INDONESIA',
                                        'PT. HONDA PROSPECT MOTOR',
                                        'PT. MITRA METAL PERKASA',
                                        'PT. NICHIRIN INDONESIA',
                                        'PT. SANSHO RUBBER INDONESIA',
                                        'PT. DNP INDONESIA',
                                        'PT. MOTHERSON YACHIO',
                                        'PT. ASAMA INDONESIA MANUFACTURING'
                                    ] as $pt)
